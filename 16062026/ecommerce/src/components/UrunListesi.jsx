...
export default function UrunListesi({
  ...
  // products dizisini kullanıcının yazdığı arama kelimesine göre süzüp filteredProducts içine attık
const filteredProducts = products.filter item =>
  // ürünün adını küçük harfe çevirip aranan kelimeyi içeriyor mu diye baktık
  item.ad.toLowerCase.includes searchTerm.toLowerCase ||
  // veya ürünün markasını küçük harfe çevirip aranan kelimeyi içeriyor mu diye kontrol ettik
  item.marka.toLowerCase.includes searchTerm.toLowerCase
;
  return (// onClick butona fareyle basılınca tetiklenir: onCategoryChange fonksiyonuna reyon adını fırlatır
// className içindeki ters tırnaklar metin ile şart kodunu tek satırda birleştirir
// ${...} dolar süslü alanı içine react kodu yani aktif pasif renk şartını yazarız
// activeCategory === Giyim satırı o an tıklanan reyonun hafızadakiyle aynı olup olmadığını test eder
// ? işareti şart doğruysa butona aktif mavi css sınıfını yapıştırır rengini açar
// : işareti şart yanlışsa sönük durması için gri pasif css sınıfını butona yapıştırır
                      ...
                      <div className="urun-fiyat-grubu">
                        <div>
                          {item.eskiFiyat && (
                            ...
                            </span>// Üstteki süslü parantezin bittiği bu alt satırda eski fiyat şartlı listelemesini bitirdik
                          )}
                        ...
                        <button
                          ...
                          className="urun-sepet-ekle-butonu" // Üstteki buton etiketinin bittiği bu alt satırda kart içi bağımsız sepet işlemini sonlandırdık
                        > 
